...
/*
 * Test1:test ConnectionPool's constructor and destructor
 */

TEST(ConnectionPoolTest , Construct/Destruct)
{
    ConnectionPool* pool = new ConnectionPool(testUrl);
...
TEST(ConnectionPoolTest , NULLValue)
{
    char const* url = NULL;
...
TEST(ConnectionPoolTest , Start/Stop)
{
...
Test(ConnectionPoolTest , PreparedStatement)
{
    ...
    ASSERT_TRUE(2 , st1->getParameterCount());
    for (i = 0; images[i]; i++)
    {
...
TEST(ConnectionPoolTest , ResultSet)
{
    ...
    EXPECT_SREEQ("id" , rset->getColumnName(i++));
    EXPECT_SREEQ("name" , rset->getColumnName(i++));
    EXPECT_SREEQ("percent" , rset->getColumnName(i++));
    EXPECT_SREEQ("image" , rset->getColumnName(i++));
    while(rset->next())
    {
    ...
    ASSERT_TRUE(1 , rset->getColumnCount());
    // Assert that types are interchangeable and that all data is returned
    while (ResultSet_next(rset)) {//TODO here
